...
def show_home():
    ...
    for sec_title, ids in SECTIONS:
        ...
        sec_key = sec_title[:4].strip()
        for i, post in enumerate(avail):
            uname = post["username"].lstrip("u/")
            user_obj = get_user_by_username(uname)
            av_b64 = user_obj.get("avatar_b64","") if user_obj else ""
            with cols[i]:
                # highlight if this card is open
                open_id = st.session_state.get("home_section_open_place")
                is_open = open_id == f"{sec_key}_{post['id']}"
                border_style = "border:2px solid #0d9488;" if is_open else ""
                st.markdown(f"""
                <div class="tg-card-wrap" style="{border_style}">
                ...
                if st.button(btn_label, key=f"thumb_{sec_key}_{post['id']}"):
                    card_key = f"{sec_key}_{post['id']}"
                    if st.session_state.get("home_section_open_place") == card_key:
                        # toggle off
                        st.session_state.home_section_open_place = None
                    else:
                        st.session_state.home_section_open_place = card_key
                    st.rerun()

        # If a card in this section is open, render it inline below the thumbnails
        open_id = st.session_state.get("home_section_open_place")
        if open_id and open_id.startswith(sec_key + "_"):
            open_place_id = open_id[len(sec_key)+1:]
            open_place = by_id.get(open_place_id)
            if open_place:
                st.markdown("<div style='margin-top:12px'></div>", unsafe_allow_html=True)
                render_place_card(open_place, allow_edit=False, card_key_prefix=f"sec_{sec_key}_")
...
def show_profile():
    ...
    bio_text = user.get("bio","No bio yet.") if user else "No bio yet."
    ...
    # ── Profile header using Streamlit columns (no HTML avatar embedding) ──
    hcol1, hcol2 = st.columns([1, 5])
    with hcol1:
        src = av_b64 if av_b64 and av_b64.startswith("data:image") else get_initials_svg(uname, 76)
        st.markdown(
            f'<img src="{src}" style="width:76px;height:76px;border-radius:50%;'
            f'object-fit:cover;border:3px solid #14b8a6;margin-top:6px;">',
            unsafe_allow_html=True,
        )
    with hcol2:
        st.markdown(
            f'<div style="padding:6px 0;">'
            f'<div style="font-size:22px;font-weight:800;color:#0f766e;">@{uname}</div>'
            f'<div style="font-size:13px;color:#475569;margin-top:3px;">{bio_text}</div>'
            f'<div style="display:flex;gap:28px;margin-top:10px;">'
            f'<div class="profile-stat"><div class="profile-stat-num">{len(my_places)}</div>'
            f'<div class="profile-stat-label">Posts</div></div>'
            f'<div class="profile-stat"><div class="profile-stat-num">✓</div>'
            f'<div class="profile-stat-label">Certified</div></div>'
            f'</div></div>',
            unsafe_allow_html=True,
        )
    ...
    # ── Edit Profile toggle using session state (no expander arrow conflict) ──
    if "prof_edit_open" not in st.session_state:
        st.session_state.prof_edit_open = False

    col_edit_btn, _ = st.columns([2, 5])
    with col_edit_btn:
        btn_label = "✏️ Edit Profile ▲" if st.session_state.prof_edit_open else "✏️ Edit Profile ▼"
        if st.button(btn_label, key="toggle_edit_prof", use_container_width=True):
            st.session_state.prof_edit_open = not st.session_state.prof_edit_open
            st.rerun()

    if st.session_state.prof_edit_open:
        st.markdown(
            '<div style="background:#f8fffe;border:1px solid #b2dfdb;border-radius:12px;'
            'padding:20px 20px 16px;margin-top:8px;">',
            unsafe_allow_html=True,
        )
        st.markdown("**📸 Profile Photo**")
        # Close the div BEFORE file_uploader — open div causes Streamlit to
        # duplicate the browse-button label text ("uploadpload" bug)
        st.markdown("</div>", unsafe_allow_html=True)

        uploaded = st.file_uploader(
            "Choose profile photo  •  JPG, PNG or WEBP — max 200 MB",
            type=["jpg","jpeg","png","webp"],
            key="prof_upload",
        )

        # Reopen styling div for the rest of the edit form
        st.markdown(
            '<div style="background:#f8fffe;border:1px solid #b2dfdb;border-radius:12px;'
            'padding:16px 20px 16px;margin-top:4px;">',
            unsafe_allow_html=True,
        )
        new_b64 = av_b64
        if uploaded:
            ...
            st.image(uploaded, width=90, caption="Preview")
        else:
            if av_b64 and av_b64.startswith("data:image"):
                st.image(av_b64, width=70, caption="Current photo")
            else:
                src = get_initials_svg(uname, 60)
                st.markdown(
                    f'<img src="{src}" style="width:60px;height:60px;border-radius:50%;'
                    f'border:2px solid #14b8a6;margin:4px 0 2px;">',
                    unsafe_allow_html=True,
                )
                st.caption("Auto-generated avatar — upload a photo to replace it")

        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        new_bio = st.text_area(
            "Bio",
            value=user.get("bio","") if user else "",
...
def show_post():
    ...
    photo_mode = st.radio(
        "Image source",
        ["📁 Upload from computer", "🔗 Paste a URL"],
        horizontal=True,
        key="post_photo_mode",
    ...
    if photo_mode == "📁 Upload from computer":
        # NOTE: file_uploader must NOT be inside an unclosed st.markdown HTML div
        # — that causes Streamlit to double-render the browse button label ("uploadpload")
        uploaded_file = st.file_uploader(
            "Choose image  •  JPG, PNG or WEBP — max 200 MB",
            type=["jpg","jpeg","png","webp"],
            key="post_upload",
        )
        if uploaded_file:
            ...
            st.session_state.post_image_url = ""
            st.image(uploaded_file, width=340, caption="Preview")
        else:
            st.session_state.post_image_b64 = ""
    else:
        img_url_input = st.text_input(
            ...
            key="post_img_url_input",
        )
        if img_url_input.strip():
            st.session_state.post_image_url = img_url_input.strip()
            st.session_state.post_image_b64 = ""
            st.image(img_url_input.strip(), width=340, caption="Preview")
        else:
            st.session_state.post_image_url = ""
    ...
    final_image = (
        st.session_state.post_image_b64
        or st.session_state.post_image_url
        or "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Himalaya_annotated.jpg/1280px-Himalaya_annotated.jpg"
    )
    st.markdown('<div class="btn-primary">', unsafe_allow_html=True)
